...
const regex = /const dynamicInteractions = analyzeInteractionsDynamic\(allBranches, daewunBranch, seunBranch\);[\s\S]*?if \(lang === 'KO' && koNotes\.length > 0\) \{/g;

const replacement = `const dynamicInteractions = analyzeInteractionsDynamic(allBranches, daewunBranch, seunBranch);
    if (dynamicInteractions.length > 0) {
      ...
      groupedInteractions.forEach((int: any) => {
        ...
        const formatB = (branch: string) => {
          ...
          if (koName.length > 1) koName = koName.substring(0, 1);
        ...
        const formTitleKO = int.cycle === 'daewun x seun' 
           ...
           : (int.cycle === 'daewun' ? \`사주 원국의 \${getBranchNameKO(int.natalIndices)} \${formatB(int.natalBranches[0])}와 대운 \${formatB(int.cycleBranch)}\`
                                     : \`사주 원국의 \${getBranchNameKO(int.natalIndices)} \${formatB(int.natalBranches[0])}와 세운 \${formatB(int.cycleBranch)}\`);
