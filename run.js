  ...
  '辛丑': {
    ...
    narrative_blocks: {
      ...
      timing_modifiers: {
        ...
        earth: '견고한 흙무덤이 더욱 굳어져, 외부와 단절된 나만의 철옹성에서 집요하게 은밀한 성장에 몰두합니다.',
...
for (const [key, value] of Object.entries(newIljus)) {
    const isExisting = content.includes(`kr_name: '${value.metadata.kr_name}'`);
    ...
    if (isExisting) {
        ...
        const rx = new RegExp(`('${key}'|"${key}"): \\{[\\s\\S]*?narrative_blocks: \\{[\\s\\S]*?\\}\\s*?\\},?`);
