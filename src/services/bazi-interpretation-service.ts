...
export function generateBaziInterpretation(result: BaZiResult, userInput: UserInput, lang: string = 'KO'): BaziInterpretationData {
  ...
  const currentDaeun = result.dynamic_luck?.current_daewoon || 
  ...
  const currentSewun = result.dynamic_luck?.current_seun || (lang === 'KO' ? "2026 병오년 (丙午)" : "2026 Year of Fire Horse (丙午)");
