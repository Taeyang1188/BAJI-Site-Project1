...
let finalScript = content.substring(0, startIndex) + newCore + content.substring(logicEndIdx);

// V5.4 add the Innate/Dynamic separation to the UI output text
const textReplacements = [
    { target: "현재 두 분의 싱크로율 수치는 서로의 에너지 방향성이 완벽하게 일치하여", replace: "선천적 원국과 대운을 종합한 두 분의 싱크로율은 압도적입니다. 서로의 존재만으로 빛이 나는 천생연분임을 의미하며, 특히 선천적 결속력이 " },
];
