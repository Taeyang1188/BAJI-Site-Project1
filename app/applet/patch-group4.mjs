...
for (const [key, value] of Object.entries(newIljus)) {
  const isExisting = ds.includes(`kr_name: '${value.metadata.kr_name}'`);
  ...
  if (isExisting) {
    ...
    ds = ds.replace(rx, block);
  } else {
    ds = ds.replace('export const ILJU_DATASET: Record<string, IljuData> = {', `export const ILJU_DATASET: Record<string, IljuData> = {\n${block}`);
  }
}
