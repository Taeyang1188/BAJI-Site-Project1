...
for (const field of fieldsToWrap) {
  // Use simple string search instead of complex regexes to avoid parsing issues
  // We'll replace instances of "field: '" and "field: \\""
  // Actually, we can use a simpler regex that doesn't use escaped quotes
  const r1 = new RegExp(field + ":\\\\s*'(.*?)'", 'g');
  tsCode = tsCode.replace(r1, function(m, p1) {
      if (m.indexOf("\\\\n") > -1) return m; // multi-line strings skip to avoid issues, we shouldn't have them in our dataset mostly
