...
// 1. Fix timeline-briefing-service.ts
...
const rep = `           let pastTenGod = prevDaewunDiff !== -1 ? tenGodsKo[prevDaewunDiff].split('(')[0] : "알 수 없는 기운";
...
if (regex.test(ts)) {
    ts = ts.replace(regex, rep);
    fs.writeFileSync('src/services/timeline-briefing-service.ts', ts, 'utf8');
    console.log('Successfully fixed timeline overlap!');
} else {
    console.log('Regex fail in timeline');
}

// 2. Fix Relationship dynamic string and syntaxes!
...
dynam = dynam.replace(voidRegex, "${subject}의 오행 중 부족한 글자(${targetElStr})에 대한 갈증을 ${target}의 강력한 ${targetElStr} 기운이 채워줍니다. '내가 결코 가질 수 없는 지도의 빈칸'이 완벽히 메워지는 엄청난 시너지를 경험하게 됩니다.");
            ...
            if (isKO) {
                ...
                if (uBenefitScore > 10 && pBenefitScore > 10) {
                    ...
                    resultText += \\\`\\\\n\\\\n[일방적 에너지 수혈] 상대방이 당신의 절대적인 부족함을 묵묵히 채워주는 맹목적인 관계입니다. 내겐 구원자와 같아서 상대의 넉넉한 \\\${getKo(pTopGive)} 기운 덕분에 내가 심리적 안정감을 얻고 살아나지만, 반대로 상대방은 에너지가 소진될 수 있으니 당신의 끝없는 감사와 배려가 필수적입니다. \\\`;
                     ...
                     resultText += \\\`\\\\n\\\\n[독립적 동행] 특별히 한쪽이 기운을 전적으로 내어주고 받기보다 두 사람 모두 고유의 독립적인 기운이 강합니다. 서로에게 일방적으로 의존하거나 바꾸려 하기보다는, 각자의 고유한 영역을 존중하고 적절한 거리감을 허용할 때 가장 훌륭한 텐션과 밸런스를 유지합니다. \\\`;
                }
...
\`;
    ...
    console.log('Successfully fixed dynamics');
} else {
    console.log('Marker temp not found in dynamics');
}
