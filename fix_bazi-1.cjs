...
const start_idx = 4066;
const end_idx = 4422;

const replacement = `-sm text-white/80 leading-relaxed font-sans">
              ...
              {guideStep === 6 && (
                <div className="flex flex-col gap-6">
                  {(() => {
                    const pillarMap = { 'Year': 0, 'Month': 1, 'Day': 2, 'Hour': 3 };
                    ...
                    const rootStems = result.analysis?.rootingInfo?.roots[stem] || [];
                    ...
                    const getTenGodBehavior = (tenGodKo, lang, pillar, damageInfo, targetType, stemChar, userName, rootType) => {
                      const baseBehaviors = {
                        '비견': {
...
const newLines = [...lines.slice(0, start_idx), replacement, ...lines.slice(end_idx)];
